...
Bitbucket PR 代码审查插件，支持按固定审查维度输出预览或发布评论。

## 功能

- 解析 Bitbucket PR URL，提取项目、仓库、PR 信息
- 按 5 个固定维度审查代码变更
- 置信度评分过滤低质量问题
- 支持 `--dry-run` 预览与正式发布评论
...
使用 `/bb-review <URL>` 或 `$bb-code-review <URL>` 去掉 `--dry-run` 后发布评论
```
