...
git clone <repo-url> .claude-plugins/gerp-tools
```

启动 Claude Code 时指定插件目录：

```bash
claude --plugin-dir .claude-plugins/gerp-tools/plugins/gerp-commit
```
...
```json
{
  "plugins": [".claude-plugins/gerp-tools/plugins/gerp-commit"]
}
```
...
/plugin install gerp-commit@gerp-tools
```
